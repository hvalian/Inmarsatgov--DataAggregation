...
namespace Aggregation_Library
{
    public class Aggregation
    {
        ...
        #region Properties
        private static bool SkipJobProcessing
        {
            get
            {
                return (ConfigurationManager.AppSettings["skipJobProcessing"] == "1");
            }
        }
        ...
        private static void CleanUp(igenmsEntities context, tbJob job)
        {
            ...
            foreach (string path in paths)
            {
                ...
                foreach (string folder in folders)
                {
                    ...
                    {
                        DateTime fileCreatedDate = File.GetCreationTime(fileName).ToUniversalTime();
        ...
        private static int CreateJob(igenmsEntities context, tbJob lastJob)
        {
            ...
            try
            {
                ...
                tbJob jobExist = (from j in context.tbJobs
                                  where j.aggregationStartDateTime <= aggregationStartDateTime &&
                                    j.aggregationEndDateTime <= aggregationEndDateTime &&
                                    j.status == (int)Status.Created &&
                                    (j.jobType == (int)Job_Type.Normal || j.jobType == (int)Job_Type.Rerun)
                                  orderby j.priority, j.aggregationStartDateTime ascending, j.startAfterDateTime ascending, j.id ascending
                                  select j).FirstOrDefault();
        ...
        private static ConfigObject GetConfiguration(igenmsEntities context, DateTime? date)
        {
            ...
            try
            {
                ...
                foreach (tbConfiguration item in listOfConfiguration)
                {
                    ...
                    {
                        configuration.Disabled = (item.value == "1");
                    }
                    ...
                    if (item.key == "ReRun_StartTimeDelay")
                    {
                        configuration.ReRun_StartTimeDelay = Int32.Parse(item.value);
                    }
            ...
            if (IsEnabled())
            {
                try
                {
                    if (jobId == 0)
                    {
                        ...
                        job = (from j in context.tbJobs
                               where j.startAfterDateTime <= currentTime &&
                                j.status == (int)Status.Created &&
                                j.aggregationStartDateTime <= configuration.Job_SuspendProcessingAfter
                               orderby j.priority, j.aggregationStartDateTime ascending, j.startAfterDateTime ascending, j.id ascending
                               select j).FirstOrDefault();
                    }
                    else
        ...
        public static int GetNextJobDelay()
        {
            ...
            try
            {
                using (igenmsEntities context = new igenmsEntities())
                {
                    ...
                    tbJob job = (from j in context.tbJobs
                                 where j.startAfterDateTime <= currentTime && j.status == (int)Status.Created && j.aggregationStartDateTime <= configuration.Job_SuspendProcessingAfter
        ...
        private static bool JobExist(igenmsEntities context)
        {
            ...
            tbJob job = (from j in context.tbJobs
                         where j.startAfterDateTime <= currentTime && 
                            j.status == (int)Status.Created && 
                            (j.jobType == (int)Job_Type.Normal || j.jobType == (int)Job_Type.Rerun) && 
                            j.aggregationEndDateTime <= configuration.LastHourlyJob_Scheduled_DateTime
                         orderby j.priority, j.aggregationStartDateTime ascending, j.startAfterDateTime ascending, j.id ascending
                         select j).FirstOrDefault();
            ...
            try
            {
                using (igenmsEntities context = new igenmsEntities())
                {
                    ...
                    if (!configuration.Disabled)
                    {
                        ...
                        if (!processStated && logger.returnCode == 0)
                        {
                            logger.returnCode = -1;
                    ...
                    {
                        logger.returnCode = -2;
                        logger.exceptionMessage = "Process is disabled.";
                    }
                }
            }
            catch (Exception ex)
        ...
        private static int MergeRefTables(igenmsEntities context, tbJob job)
        {
            Logger logger = new Logger(job.id, "MergeRefTables", null);
            ...
            try
            {
                ...
                var data = context.Database.ExecuteSqlCommand("exec @ReturnCode = [aggregation].[spMergeRefTables] ", parameter.ToArray());
        ...
        {
            Logger logger = new Logger(job.id, "PopulateDaily", null);
        ...
        {
            Logger logger = new Logger(job.id, "PopulateHourly", null);
        ...
        private static int PopulateStaging(igenmsEntities context, tbJob job)
        {
            string parmeters = "Job Id:" + job.id.ToString();
            Logger logger = new Logger(job.id, "PopulateStaging", null);
            ...
            try
            {
                ...
                var data = context.Database.ExecuteSqlCommand("exec @ReturnCode = [aggregation].[spPopulateStaging] @jobId, @fromTimeStamp, @toTimeStamp ", parameter.ToArray());
        ...
        private static int ReRun(igenmsEntities context, tbJob lastJob)
        {
            try
            {
                tbJob job = new tbJob();
                job.aggregationStartDateTime = lastJob.aggregationStartDateTime;
                job.aggregationEndDateTime = lastJob.aggregationEndDateTime;
                job.startAfterDateTime = System.DateTime.Now.AddMinutes(configuration.ReRun_StartTimeDelay);
                job.priority = lastJob.priority;
                job.interval = lastJob.interval;
                job.jobType = (int)Job_Type.Rerun;
                job.isRecovery = false;
                job.createdDateTime = System.DateTime.Now;
                job.createdBy = GetCurrentUserName();
                job.status = (int)Status.Created;
        ...
        private static void SendNotification(AggregationObject aggregationObj)
        {
            ...
            string messageBody = @"Aggregation job was unsuccessful." + '\n' + '\n';
            ...
            try
            {
                ...
                message.Subject = "Aggregation Alert (" + configuration.ProjectName + ") " + String.Format("Job # {0} was unsuccessful", aggregationObj.jobId.ToString());
        ...
        private static int StartJob(igenmsEntities context, long jobId)
        {
            ...
            try
            {
                ...
                {
                    logger.returnCode = MergeRefTables(context, job);

                    if (logger.returnCode == 0 && job.interval == (int)Interval.Hourly)
                    {
                        logger.returnCode = PopulateStaging(context, job);
                    }
                    ...
                    if (logger.returnCode == 0)
                    {
                        ...
                        if (logger.returnCode == 0)
                        {
                            ...
                            if (job.processed == 0)
                            {
                                if (job.interval == (int)Interval.Hourly)
                                {
                                    string errMessage = "There is no data for " + job.aggregationStartDateTime + " to " + job.aggregationEndDateTime + '.' + '\n';
                                    errMessage += "ReRun will start in " + configuration.ReRun_StartTimeDelay.ToString() + " minutes." + '\n';

                                    job.status = (int)Status.Abend;
                                    logger = new Logger(job.id, "GetNumberOfProcessedRecords", null);
                                    logger.returnCode = -4;
                                    logger.exceptionMessage = errMessage;
                                    AggregationObj.errors.Add(logger);
                                }
                                else if (job.isRecovery && (bool)job.isRecovery)
                                {
                                    job.processed = 1;
                                }
                            }
                ...
                if (job.status == (int)Status.Completed || (job.status == (int)Status.Abend && job.processed == 0 && job.interval == (int)Interval.Hourly))
                {
                    CleanUp(context, job);
                    ...
                    if (job.status == (int)Status.Abend && job.processed == 0 && job.interval == (int)Interval.Hourly)
                    {
                        ReRun(context, job);
                    }
                }
            ...
            if (logger.returnCode != 0 && logger.returnCode != -4)
            {
        ...
        {
            string parmeters = "Job Id:" + job.id.ToString();
            Logger logger = new Logger(job.id, "PopulateNodesStatus", null);
            ...
            try
            {
                ...
                var data = context.Database.ExecuteSqlCommand("exec @ReturnCode = [aggregation].[spPopulateNodeStatus] @jobId, @fromTimeStamp, @toTimeStamp ", parameter.ToArray());
            ...
            try
            {
                ...
                foreach (Logger logItem in aggregationObject.errors)
                {
                    if (logItem != null)
                    {
                        ...
                        {
                            sendEmailNotification = (logItem.returnCode == -4 && configuration.ProjectName.StartsWith("DEV - MSC")) ? false : true;
                ...
                {
                    SendNotification(aggregationObject);
                }
            }
            catch (Exception ex)
            {
